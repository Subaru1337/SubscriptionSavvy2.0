...
export default function AnimatedBar({ value, maxValue, label, delay = 0, isCurrentMonth = false }: { value: number, maxValue: number, label: string, delay?: number, isCurrentMonth?: boolean }) {
  ...
  return (
    <View className="items-center flex-1 justify-end h-full">
      {value > 0 && (
        <Text className="text-[9px] font-bold text-[#6B7280] mb-1.5" style={{ fontFamily: 'PlusJakartaSans_700Bold' }}>₹{value.toFixed(0)}</Text>
      )}
